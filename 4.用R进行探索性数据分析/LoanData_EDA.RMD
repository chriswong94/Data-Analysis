...
1.贷款数据集包含了太多数据，而且每项贷款有81个变量。可供选择的变量太多了，并且因为没有接触过相关业务，导致对数据理解很差，不知道应该选用哪些数据。选用的可视化方法也相应较少，应该加强对相关资料的检索与练习。

2.统计学知识比较薄弱，没有建立相关模型的概念，在后续的时间应该加强学习。

3.之前熟系python清洗数据的方法，在项目开头也相应的用R处理了一些变量。
